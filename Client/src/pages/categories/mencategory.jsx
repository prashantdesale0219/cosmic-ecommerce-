  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      const mockProducts = Array(16).fill().map((_, index) => ({
        id: `men-product-${index + 1}`,
        name: `Men's ${['Premium T-Shirt', 'Slim Fit Shirt', 'Denim Jeans', 'Casual Trousers', 'Bomber Jacket'][index % 5]} ${index + 1}`,
        price: Math.floor(Math.random() * 3000) + 999,
        originalPrice: Math.floor(Math.random() * 4000) + 1499,
        discount: Math.floor(Math.random() * 40) + 10,
        image: productImages[index % productImages.length],
        slug: `men-product-${index + 1}`,
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="text-sm breadcrumbs mb-6">
        <ul className="flex space-x-2">
          ...
          <li className="text-gray-900 font-medium">Men</li>
        </ul>
      </div>
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600">Discover our latest men's fashion collection with premium quality and stylish designs.</p>
      </div>
